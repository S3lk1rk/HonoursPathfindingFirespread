...
def reconstruct_path(came_from, start, end, draw=None):
    ...
    if draw:  # Display the text only if the draw function is provided
        text_x, text_y = WIN.get_size()
        text_x //= 2
        text_y //= 2
        draw_text(WIN, "Successfully reached destination!", 40, WHITE, text_x, text_y)
        pygame.display.update()
        pygame.time.delay(2000)  # Wait for 2 seconds before proceeding
...
def draw_text(win, grid, rows, width):
    has_purple_tile = False
    for row in grid:
        for spot in row:
            if spot.color == PURPLE:
                has_purple_tile = True
            spot.draw(win)
    ...
    if has_purple_tile:
        ...
        text = font.render("Destination Reached!", True, WHITE)
