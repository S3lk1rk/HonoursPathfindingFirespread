...
def draw_text(win, text, rows, width):
    pygame.font.init()
    font = pygame.font.SysFont("comicsans", 30)
    text_surface = font.render(text, True, WHITE)
    text_rect = text_surface.get_rect()
    text_rect.center = (width // 2, rows // 2)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and start and end:
                    ...
                    came_from, path_found = algorithm(lambda: draw(win, grid, ROWS, width), grid, start, end)
                    if path_found:
                    ...
                    if destination_reached:
                        # Change background color to grey and display text
                        for row in grid:
                            for spot in row:
                                spot.color = GREY
                        draw_text(win, "Destination Reached!", ROWS, width)
                        time.sleep(3)
                        destination_reached = False
